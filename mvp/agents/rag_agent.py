...
class RAGAgent(BaseAgent):
    ...
    def process(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if not chunks:
                ...
                no_context_msg = self._get_no_context_message(state.language_config)
    ...
    def _get_no_context_message(self, language_config: Dict) -> str:
        """Mensaje cuando no hay contexto relevante."""
        if not language_config:
            return "No encontré información relevante para tu pregunta."
        
        return language_config.get('no_context', 'No encontré información relevante para tu pregunta.')
